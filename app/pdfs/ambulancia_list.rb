class AmbulanciaList < Prawn::Document
  ...
  def initialize(ambulancias, view)
     super()
     logo 
  ...
  def logo
    ...
    move_down 10
    draw_text "Listado de ambulancias", :at => [150, 575], size: 22
    text "SAVE DRIVING"
  ...
  def ambulancia_item_rows
    [["id", "documento", "numero", "nombres", "direccion", "ciudad", "telefonos", "email", "observaciones", "calificacion" ]] +
    @ambulancias.map do |ambulancia|
      [ "#{ambulancia.id} ",   
      "#{ambulancia.documento.nombre} ", 
      "#{ambulancia.numero_doc} ",
      "#{ambulancia.nombres} ", 
      "#{ambulancia.direccion} ", 
      "#{ambulancia.municipio.nombre} ", 
      "#{ambulancia.telefonos} ", 
      "#{ambulancia.email} ",
      "#{ambulancia.observaciones} ", 
      "#{ambulancia.calificacion.nombre} "]
    end
  end
  
  #Método que imprime la tabla de las ordenes que hay
  def ambulancia_details
    move_down 80
    table ambulancia_item_rows, :width => 532 do
