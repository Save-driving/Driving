class AbogadoList < Prawn::Document
  ...
  def logo
    logopath =  "#{Rails.root}/app/assets/images/title2.png"
    image logopath, :width => 250, :height => 64
    move_down 10
    draw_text "Listado de Abogados", :at => [150, 575], size: 22
  end
  ...
  def abogado_item_rows
    [["Id", "Doc", "Num", "Nombre", "Dir", "Ciudad", "Tels", "Email", "Obs", "Calif" ]] +
    @abogados.map do |abogado|
      [ "#{abogado.id} ",   
      "#{abogado.documento.nombre} ", 
      "#{abogado.numero_doc} ",
      "#{abogado.nombres} ", 
      "#{abogado.direccion} ", 
      "#{abogado.municipio.nombre} ", 
      "#{abogado.telefonos} ", 
      "#{abogado.email} ",
      "#{abogado.observaciones} ", 
      "#{abogado.calificacion.nombre} "]
    end
  end
  ...
  def abogado_details
    move_down 80
    table abogado_item_rows, :width => 532 do
