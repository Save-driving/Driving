class AseguradoraList < Prawn::Document
  ...
  def initialize(aseguradoras, view)
     super()
     logo 
  ...
  #    }
  def to_pdf(aseguradoras, view)
    logo
    @aseguradoras = aseguradoras
    @vista = view
    aseguradoras_details
    render
  end
  ...
  def logo
    ...
    move_down 10
    draw_text "Listado de aseguradoras", :at => [150, 575], size: 22
    text "SAVE DRIVING"
  ...
  def aseguradora_item_rows
    [["id", "documento", "numero", "nombres", "direccion", "ciudad", "telefonos", "email", "observaciones", "calificacion" ]] +
    @aseguradoras.map do |aseguradora|
      [ "#{aseguradora.id} ",   
      "#{aseguradora.documento.nombre} ", 
      "#{aseguradora.numero_doc} ",
      "#{aseguradora.nombres} ", 
      "#{aseguradora.direccion} ", 
      "#{aseguradora.municipio.nombre} ", 
      "#{aseguradora.telefonos} ", 
      "#{aseguradora.email} ",
      "#{aseguradora.observaciones} ", 
      "#{aseguradora.calificacion.nombre} "]
    end
  end
  
  #Método que imprime la tabla de las ordenes que hay
  def aseguradora_details
    move_down 80
    table aseguradora_item_rows, :width => 532 do
