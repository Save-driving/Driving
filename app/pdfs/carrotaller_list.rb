class CarrotallerList < Prawn::Document
  ...
  def initialize(carrotalleres, view)
     super()
     logo 
     @carrotalleres = carrotalleres
     @vista = view
     carrotaller_details
  end
  ...
  #    }
  def to_pdf(carrotalleres, view)
    logo
    @carrotalleres = carrotalleres
    @vista = view
    carrotalleres_details
    render
  end
  ...
  def logo
    ...
    move_down 10
    draw_text "Listado de carrotalleres", :at => [150, 575], size: 22
    text "SAVE DRIVING"
  ...
  def carrotaller_item_rows
    [["id", "documento", "numero", "nombres", "direccion", "ciudad", "tel", "email", "observaciones", "calificacion"]] +
    @carrotalleres.map do |carrotaller|
      [ "#{carrotaller.id} ",   
      "#{carrotaller.documento.nombre} ", 
      "#{carrotaller.numero_doc} ",
      "#{carrotaller.nombres} ", 
      "#{carrotaller.direccion} ", 
      "#{carrotaller.municipio.nombre} ", 
      "#{carrotaller.telefonos} ", 
      "#{carrotaller.email} ",
      "#{carrotaller.observaciones} ",
      "#{carrotaller.calificacion.nombre} "]
    end
  end
  
  #Método que imprime la tabla de las ordenes que hay
  def carrotaller_details
    move_down 70
    table carrotaller_item_rows, :width => 520 do
