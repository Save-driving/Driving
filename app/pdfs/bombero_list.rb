class BomberoList < Prawn::Document
  ...
  def logo
    ...
    move_down 10
    draw_text "Listado de bomberos", :at => [150, 575], size: 22
    text "SAVE DRIVING"
  ...
  def bombero_item_rows
    [["id", "documento", "numero", "nombres", "direccion", "ciudad", "telefonos", "email", "observaciones", "calificacion" ]] +
    @bomberos.map do |bombero|
      [ "#{bombero.id} ",   
      "#{bombero.documento.nombre} ", 
      "#{bombero.numero_doc} ",
      "#{bombero.nombres} ", 
      "#{bombero.direccion} ", 
      "#{bombero.municipio.nombre} ", 
      "#{bombero.telefonos} ", 
      "#{bombero.email} ",
      "#{bombero.observaciones} ", 
      "#{bombero.calificacion.nombre} "]
    end
  end
  ...
  def bombero_details
    move_down 80
    table bombero_item_rows, :width => 532 do
